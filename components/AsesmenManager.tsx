...
const AsesmenManager: React.FC<AsesmenManagerProps> = ({ type, user }) => {
  ...
  const renderSoalContent = (content: string, isPrint = false) => {
        ...
        renderedParts.push(
          <div key={key} className="whitespace-pre-wrap text-justify leading-relaxed mb-4">
            {currentParagraphLines.map((line, li) => {
              ...
              return (
                <div key={li} dir={isArabic ? 'rtl' : 'ltr'} className={`${isOption ? 'block pl-6 -indent-6 mb-2 font-bold' : ''} ${isArabic ? 'text-right font-serif text-2xl leading-loose my-3' : ''}`}>
                  {line}
                </div>
              );
            })}
        ...
        renderedParts.push(
          <div key={key} className="overflow-x-auto my-5">
            <table className={`border-collapse border-2 border-black w-full shadow-sm ${isPrint ? 'text-[9.5px]' : 'text-[11.5px]'}`}>
              <thead>
                <tr className="bg-slate-100">{rows[0].map((cell, i) => (<th key={i} className="border-2 border-black p-2 font-black text-center uppercase tracking-tight">{cell}</th>))}</tr>
              </thead>
              <tbody>
                {rows.slice(1).map((row, ri) => (
                  <tr key={ri} className="hover:bg-slate-50 transition-colors">
                    {row.map((cell, ci) => {
                      ...
                      return (<td key={ci} dir={isArabic ? 'rtl' : 'ltr'} className={`border-2 border-black p-2.5 ${ci === 0 && rows[0].length > 1 ? 'bg-slate-50/50 font-bold text-center w-12' : ''} ${isArabic ? 'text-3xl font-serif leading-loose text-center py-5' : 'text-left'}`}>{cell}</td>);
                    })}
                  </tr>
                ))}
              </tbody>
    ...
    lines.forEach((line, index) => {
      ...
      } else if (isSeparator) {} else {
        if (currentTableRows.length > 0) flushTable(`t-${index}`);
        if (trimmedLine.length > 0) { currentParagraphLines.push(line); } else if (currentParagraphLines.length > 0) { flushParagraph(`p-${index}`); }
      }
    });
                  ...
                  {filteredKisikisi.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="border-2 border-black p-2 leading-relaxed">
                         {item.stimulus && (<div className="mb-2 p-2 bg-slate-50 border border-slate-200">{renderSoalContent(item.stimulus, true)}</div>)}
                  ...
                  {filteredKisikisi.map((item) => (
                    <tr key={item.id} className="break-inside-avoid">
                      ...
                      <td className="pb-12 align-top">
                         {item.stimulus && (<div className="mb-6"><p className="font-bold text-[11px] italic text-slate-700 mb-2 tracking-tight">Cermatilah teks/tabel berikut untuk menjawab soal!</p><div className="p-6 border-[1.5px] border-black bg-slate-50 italic text-[12.5px] leading-relaxed shadow-sm">{renderSoalContent(item.stimulus, true)}</div></div>)}
                         <div className="text-[13.5px] leading-relaxed">{renderSoalContent(item.soal, true)}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden min-h-[400px]">
        {loading ? (<div className="flex flex-col items-center justify-center py-40 gap-4 text-slate-400"><Loader2 size={48} className="animate-spin text-rose-600" /><p className="font-black text-xs uppercase tracking-widest">Sinkronisasi Cloud...</p></div>) : activeTab === 'KISI_KISI' ? (
                ...
                {filteredKisikisi.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50 align-top transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <select className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-[10px] font-bold mb-1" value={item.tpId} onChange={e => updateKisiKisi(item.id, 'tpId', e.target.value)}><option value="">Pilih TP</option>{tps.filter(t => t.kelas === kelas && t.mataPelajaran === mapel).map(t => <option key={t.id} value={t.id}>{t.tujuanPembelajaran}</option>)}</select>
                      <div className="text-[9px] text-slate-500 italic leading-tight">{item.tujuanPembelajaran}</div>
                    </td>
                    <td className="px-6 py-4"><select className="w-full bg-indigo-50 border border-indigo-100 rounded-xl p-2.5 text-[10px] font-black text-indigo-700 outline-none" value={item.kompetensi} onChange={e => updateKisiKisi(item.id, 'kompetensi', e.target.value as any)}><option value="Pengetahuan dan Pemahaman">Pengetahuan dan Pemahaman</option><option value="Aplikasi">Aplikasi</option><option value="Penalaran">Penalaran</option></select></td>
                    <td className="px-6 py-4 text-center"><select className="w-full text-[10px] font-bold p-1.5 border rounded-xl bg-slate-50 outline-none" value={item.bentukSoal} onChange={e => updateKisiKisi(item.id, 'bentukSoal', e.target.value as any)}><option>Pilihan Ganda</option><option>Pilihan Ganda Kompleks</option><option>Menjodohkan</option><option>Isian</option><option>Uraian</option></select></td>
                    <td className="px-6 py-4 relative group"><textarea className="w-full bg-white border border-slate-200 rounded-xl p-2 text-[10px] font-bold italic leading-relaxed min-h-[100px]" value={item.indikatorSoal} onChange={e => updateKisiKisi(item.id, 'indikatorSoal', e.target.value)} /><button onClick={() => generateIndikatorAI(item)} className="absolute bottom-6 right-8 text-indigo-600 bg-white p-1 rounded shadow-sm">{aiLoadingMap[`ind-${item.id}`] ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14}/>}</button></td>
                    <td className="px-6 py-4 bg-slate-50/30 relative group"><div className="grid grid-cols-2 gap-4 h-full"><div className="flex flex-col h-full"><div className="flex items-center gap-1.5 text-[9px] font-black text-indigo-600 uppercase mb-2"><BookText size={12}/> Teks Bacaan / Tabel Data</div><textarea className="flex-1 w-full bg-white border border-slate-200 rounded-xl p-2 text-[10px] font-medium italic leading-relaxed min-h-[160px]" value={item.stimulus} placeholder="Teks bacaan atau Tabel Markdown..." onChange={e => updateKisiKisi(item.id, 'stimulus', e.target.value)} /><div className="mt-2 overflow-hidden max-h-40 overflow-y-auto border border-slate-100 p-1 bg-white rounded-lg">{renderSoalContent(item.stimulus)}</div></div><div className="space-y-3 flex flex-col"><div><span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Pertanyaan & Pilihan:</span><textarea className="w-full bg-white border border-slate-200 rounded-xl p-2 text-[11px] font-bold min-h-[120px]" value={item.soal} onChange={e => updateKisiKisi(item.id, 'soal', e.target.value)} placeholder="Format: A. Opsi B. Opsi..." /></div><div className="flex items-center gap-2 mt-auto"><span className="text-[9px] font-black uppercase text-slate-400">Kunci:</span><input className="flex-1 bg-white border border-slate-200 rounded-lg p-1.5 text-[10px] font-black text-indigo-600" value={item.kunciJawaban} onChange={e => updateKisiKisi(item.id, 'kunciJawaban', e.target.value)} placeholder="Kunci..." /></div></div></div><button onClick={() => generateSoalAI(item)} className="absolute bottom-6 right-4 bg-rose-600 text-white p-2.5 rounded-2xl shadow-xl hover:scale-110 active:scale-95 transition-all z-10">{aiLoadingMap[`soal-${item.id}`] ? <Loader2 size={18} className="animate-spin" /> : <Wand2 size={18}/>}</button></td>
          ...
          <div className="p-10 bg-slate-50 min-h-[600px]"><div className="max-w-4xl mx-auto"><div className="grid grid-cols-2 gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-12"><div><label className="block text-[10px] font-black text-slate-400 uppercase mb-1">Hari / Tanggal Pelaksanaan</label><input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold" value={hariTanggal} onChange={e => setHariTanggal(e.target.value)} placeholder="Contoh: Senin, 12 Juni 2024" /></div><div><label className="block text-[10px] font-black text-slate-400 uppercase mb-1">Alokasi Waktu</label><input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold" value={waktuPengerjaan} onChange={e => setWaktuPengerjaan(e.target.value)} placeholder="Contoh: 90 Menit" /></div></div><div className="space-y-1 bg-white p-1 rounded-2xl shadow-lg border border-slate-200 overflow-hidden"><table className="w-full border-collapse"><thead><tr className="bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest h-10"><th className="w-16 px-4 text-center">No</th><th className="px-6 text-left">Naskah Soal & Bacaan</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredKisikisi.map((item) => (<tr key={item.id} className="group hover:bg-slate-50/50 transition-colors"><td className="py-8 px-4 align-top text-center"><span className="inline-flex items-center justify-center w-10 h-10 bg-slate-100 text-slate-900 rounded-xl font-black text-lg shadow-sm border border-slate-200 group-hover:bg-rose-600 group-hover:text-white transition-all">{item.nomorSoal}</span></td><td className="py-8 px-6 align-top"><div className="space-y-6">{item.stimulus && (<div><p className="font-bold text-[11px] italic text-slate-600 mb-2">Cermatilah teks/tabel berikut untuk menjawab soal nomor {item.nomorSoal}:</p><div className="p-6 bg-slate-50 rounded-[2rem] border-2 border-indigo-100 shadow-sm relative overflow-hidden mb-6"><div className="absolute top-0 right-0 p-4 opacity-5 text-indigo-600"><BookText size={64}/></div><div className="text-sm leading-relaxed text-slate-700 italic">{renderSoalContent(item.stimulus)}</div></div></div>)}<div className="text-slate-800 text-sm leading-relaxed pr-10">{renderSoalContent(item.soal)}</div></div></td></tr>))}</tbody></table>{filteredKisikisi.length === 0 && (<div className="py-40 text-center text-slate-400 italic uppercase font-black text-xs tracking-widest">Belum ada butir soal yang dibuat.</div>)}</div></div></div>
        )}
      </div>
    </div>
  );
