...
const App: React.FC = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center"><Loader2 className="animate-spin text-blue-600 mb-4" size={48} /><p className="text-xs font-black text-slate-400 uppercase">STABILIZING PRODUCTION...</p></div>;
  }

  if (!user) return <LoginPage />;

  const hasNoApiKey = !user.apiKey || user.apiKey.trim().length < 25;
  const isMenuAllowedWithoutKey = (menuId: string) => (menuId === 'DASHBOARD' || (user.role === 'admin' && (menuId === 'USER' || menuId === 'SETTING')));
    ...
    <div className="min-h-screen bg-slate-50 flex overflow-hidden">
          ...
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-8 space-y-5">
              ...
              <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-3xl space-y-3">
                <label className="flex items-center gap-2 text-[10px] font-black text-indigo-600 uppercase tracking-widest"><Key size={14}/> Gemini API Key</label>
                <div className="relative">
                  <input type={showKey ? "text" : "password"} className="w-full bg-white border border-indigo-200 rounded-xl py-3 pl-4 pr-12 text-xs font-mono font-bold" value={profileFormData.apiKey} onChange={e => setProfileFormData({...profileFormData, apiKey: e.target.value})} />
        ...
        <div className="flex flex-col h-full">
          <div className="p-6 border-b shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div><h1 className="text-sm font-black text-slate-900 uppercase leading-none">SDN 5 BILATO</h1><p className="text-[8px] text-emerald-600 font-black uppercase mt-1 tracking-widest bg-emerald-50 px-1.5 py-0.5 rounded">V3.6 - STABLE</p></div>
            </div>
          </div>
          <nav className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
            {navItems.map((item) => {
              ...
              return (
                <button key={item.id} disabled={isLocked} onClick={() => { setActiveMenu(item.id as any); setIsSidebarOpen(false); }} className={`w-full flex items-center justify-between p-3 rounded-xl text-xs font-bold transition-all ${isLocked ? 'opacity-30 grayscale' : activeMenu === item.id ? `${item.bg} ${item.color} shadow-sm border` : 'text-slate-600 hover:bg-slate-50'}`}>
                  <div className="flex items-center gap-3"><span className={`${activeMenu === item.id ? item.color : 'text-slate-400'}`}>{isLocked ? <Lock size={16} /> : item.icon}</span><span>{item.label}</span></div>
          ...
          <div className="p-4 border-t shrink-0">
            <button onClick={() => setShowProfileModal(true)} className="w-full bg-slate-50 hover:bg-slate-100 rounded-2xl p-4 flex items-center gap-3 mb-4 transition-all">
              ...
              <div className="overflow-hidden text-left"><p className="text-xs font-black text-slate-900 truncate uppercase">{user.name}</p><p className="text-[9px] text-slate-400 font-bold uppercase">{hasNoApiKey ? 'KUNCI MATI' : 'KUNCI AKTIF'}</p></div>
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden h-screen">
        <header className="bg-white border-b h-16 flex items-center justify-between px-4 shrink-0 lg:px-8">
          ...
          <div className="flex items-center gap-3">
             <div className={`px-4 py-1.5 rounded-full border flex items-center gap-2 ${hasNoApiKey ? 'bg-rose-50 border-rose-100 text-rose-600' : 'bg-emerald-50 border-emerald-100 text-emerald-600'}`}>
                {hasNoApiKey ? <ShieldAlert size={14}/> : <ShieldCheck size={14}/>}<span className="text-[9px] font-black uppercase tracking-widest">{hasNoApiKey ? 'Akses Terkunci' : 'Produksi Stabil'}</span>
             </div>
          </div>
        </header>
          ...
          <div className="max-w-7xl mx-auto">
            {isCurrentMenuRestricted ? (
              <div className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in">
                 <div className="w-24 h-24 bg-rose-100 text-rose-600 rounded-[40px] flex items-center justify-center mb-8 shadow-xl"><ShieldAlert size={48}/></div>
                 <h2 className="text-2xl font-black text-slate-900 uppercase mb-4">Akses Database Dibatasi</h2>
                 <p className="text-slate-500 font-medium max-w-md leading-relaxed mb-8">Harap input <b>Gemini API Key Pribadi</b> Anda di menu Profil untuk mengaktifkan seluruh modul kurikulum sekolah.</p>
                 <button onClick={() => setShowProfileModal(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl flex items-center gap-2"><Key size={16}/> ATUR KUNCI SAYA</button>
              </div>
            ) : (
              <>
